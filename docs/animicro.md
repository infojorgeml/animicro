...
**Release:** 1.20.0 (2026-05-13). See CHANGELOG for history.
...
- **Modules**: `fade`, `slide-up`, `slide-down`, `slide-left`, `slide-right`, `skew-up`, `scale`, `blur`, `float`, `pulse`, `hover-zoom`, `spin`, `stagger`, `grid-reveal`, `highlight`, `text-fill-scroll`, `parallax`, `img-parallax`, `clip-reveal`, `magnet`, `magnetic`, `split`, `scatter`, `scramble`, `text-reveal`, `typewriter`, `page-curtain`. Each exports `init()`.
