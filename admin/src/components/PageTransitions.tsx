...
import { EASING_OPTIONS, MODULE_INFO, DEFAULT_PAGE_FADE_CONFIG, DEFAULT_PAGE_CURTAIN_CONFIG } from '../data/modules';
import type { AnimicroSettings, ModuleConfig } from '../types';

/**
 * Page Transitions tab — renders the page-fade and page-curtain modules
 * with their toggles and inline settings.
 *
 * Unlike the Modules dashboard (which drills into a separate settings page),
 * this tab keeps settings inline below each card because we only have 2
 * modules total — no need for a nested navigation.
 */
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Animate the whole page on load. These run once per page-load, not per element.
          Both modules are builder-safe (they don't run inside Bricks / Elementor / etc. editors)
          and honor <code className="text-xs bg-gray-100 px-1 py-0.5 rounded">prefers-reduced-motion</code>.
        </p>
        <p className="text-xs text-gray-400 mt-2">
          Tip: normally you only want one of the two enabled. If both are on they coexist as independent layers (the curtain animates over a body that's fading in).
        </p>
      </div>
      ...
      <div className="space-y-4">
        {pageModules.map(mod => {
          ...
          const fallback = mod.id === 'page-fade' ? DEFAULT_PAGE_FADE_CONFIG : DEFAULT_PAGE_CURTAIN_CONFIG;
          const cfg = settings.module_settings?.[mod.id] ?? fallback;
            ...
            >
              ...
              {isActive && (
                <div className="mt-4 pt-4 border-t border-brand-100 space-y-4">
                  {/* Direction selector — page-curtain only */}
                  {mod.id === 'page-curtain' && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Direction</label>
                      <p className="text-xs text-gray-400 mb-2">
                        How the curtain animates away once the page is loaded.
                      </p>
                      <select
                      ...
                      >
                        <option value="fade">Fade — overlay fades out</option>
                        <option value="slide-up">Slide Up — overlay slides off the top</option>
                        <option value="slide-down">Slide Down — overlay slides off the bottom</option>
                      </select>
                    </div>
                  )}

                  {/* Background color + logo — page-curtain only */}
                  {mod.id === 'page-curtain' && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Background color</label>
                        <p className="text-xs text-gray-400 mb-2">
                          Color of the overlay that covers the page. Use your brand color for a polished feel.
                        </p>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mb-2">
                      Hold time before the transition starts. Useful if you want to give other entry animations a moment to settle.
                    </p>
                    <input
                      type="range"
